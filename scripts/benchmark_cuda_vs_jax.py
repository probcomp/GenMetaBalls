...
def benchmark_comparison(opt_results, kernel_id=0, warmup=10, save_plot=False, grid_size=None, block_size=None, num_fmb_chunks=4):
    ...
    for view_idx in tqdm(range(len(cameras_list)), desc="GenMetaBalls"):
        ...
        for iter_idx in range(ITER_MULT):
            ...
            if iter_idx == 0:
                ...
                cuda_results.append((depth_image, confidence))  # Keep as JAX arrays (RAW depth, not filtered)
    ...
    print(f"  Time taken for {len(cameras_list) * ITER_MULT} views: {total_cuda_time_us:.2f} microseconds")
    ...
    for camera_rays in tqdm(cameras_list, desc="JAX trained render"):
        ...
        jax_results_trained.append((est_depth_np.reshape(image_size), est_alpha_np.reshape(image_size)))
    ...
    for view_idx in range(num_views_with_results):
        ...
        cuda_depth_img = np.array(cuda_depth.reshape(height, width) if len(cuda_depth.shape) == 1 else cuda_depth)
        cuda_alpha_img = np.array(cuda_alpha.reshape(height, width) if len(cuda_alpha.shape) == 1 else cuda_alpha)
    ...
    if len(all_depth_diffs) > 0:
        ...
        print(f"Depth Comparison (valid pixels: {total_valid_pixels}/{total_pixels} across {num_views_with_results} views):")
    ...
    if len(all_alpha_diffs) > 0:
        ...
        if len(valid_alpha_diffs) > 0:
            ...
            alpha_relative_error = np.mean(valid_alpha_rel_errors) if len(valid_alpha_rel_errors) > 0 else 0.0
            alpha_median_relative_error = np.median(valid_alpha_rel_errors) if len(valid_alpha_rel_errors) > 0 else 0.0
    ...
    depth_ok = depth_relative_error < depth_tolerance if (len(all_depth_diffs) > 0 and depth_relative_error is not None) else True
    ...
    if save_plot:
        ...
        create_comparison_plots(
            jax_results_trained, cuda_results, alpha_results_final, opt_results, kernel_id, kernel_name
        )
    ...
    results = {
        ...
        'alpha_relative_error': float(alpha_relative_error) if alpha_relative_error is not None else None,
...
def create_comparison_plots(jax_results, cuda_results, alpha_results_final, opt_results, kernel_id, kernel_name):
    ...
    for view_idx in range(num_views):
        ...
        cuda_depth_jax = cuda_depth.reshape(height, width) if len(cuda_depth.shape) == 1 else cuda_depth
        cuda_alpha_jax = cuda_alpha.reshape(height, width) if len(cuda_alpha.shape) == 1 else cuda_alpha
    ...
    for view_idx in range(num_views):
        ...
        if 'ref_depths' in opt_results and view_idx < len(opt_results['ref_depths']):
            ...
            ax2.axis("off")
            ax2.text(0.5, 0.5, "N/A", ha='center', va='center', fontsize=12, transform=ax2.transAxes)
            ax2.set_title(f"GT Depth (View {view_idx})", fontsize=10, fontweight="bold")
        ...
        diff_jax = cuda_depths_raw[view_idx] - jax_depths_raw[view_idx]  # FILTERED CUDA - FILTERED JAX
    ...
    plt.tight_layout()
    depth_plot_file = cache_dir / f"depth_comparison_fmbs{opt_results['num_fmbs']}_size{width}x{height}_kernel{kernel_id}.png"
    ...
    for view_idx in range(num_views):
        ...
        cuda_confidence = cuda_results[view_idx][1]  # Get confidence directly from cuda_results (JAX array)
    ...
    plt.tight_layout()
    conf_plot_file = cache_dir / f"confidence_comparison_fmbs{opt_results['num_fmbs']}_size{width}x{height}_kernel{kernel_id}.png"
...
if __name__ == "__main__":
    ...
    print(f"  GenMetaBalls:  {results['cuda_avg_time_us']:.2f} μs (kernel_id={args.kernel_id}, {results['kernel_name']})")
