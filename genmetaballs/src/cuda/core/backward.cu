...
__host__ FMBSceneGradient::FMBSceneGradient(const FMBScene<MemoryLocation::DEVICE>& fmbs,
...
__device__ void FMBSceneGradient::buffer_grads(uint32_t i, uint32_t j, uint32_t k, float d,
                                               float q) {
...
__device__ void FMBSceneGradient::accumulate_grads(uint32_t i, uint32_t j, uint32_t k, float c,
                                                   float y) {
    const auto W = intr_.width;
    const auto N = fmbs_.size();
