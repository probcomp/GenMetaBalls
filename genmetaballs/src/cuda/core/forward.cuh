...
__global__ void render_kernel_fmb_parallel(const FMBScene<MemoryLocation::DEVICE>& fmbs,
    ...
    // Thread lane_id processes FMBs at indices: lane_id, lane_id + WARP_SIZE, lane_id +
    // 2*WARP_SIZE, ... This matches the original logic exactly, just parallelized across warp
...
void render_fmbs(const FMBScene<MemoryLocation::DEVICE>& fmbs, const Blender& blender,
    ...
    switch (kernel_id) {
        ...
        case 1: {
            // FMB-parallelized kernel (warp-level reduction)
            const int total_pixels = intr.width * intr.height;
